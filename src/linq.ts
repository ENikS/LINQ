...
class Linq<T> implements Enumerable<T>, Iterable<T>, IEnumerable<T> {
    ...
    Concat(second: Iterable<T>): Enumerable<T> {
        ...
        return new Linq<T>(this, () => new SelectManyIteratror(aggregate[Symbol.iterator](), selfFn));
    }	


    Distinct<V>(keySelector: (T) => V = selfFn): Enumerable<T> {
        return new Linq<T>(this, () => new DistinctIteratror(this._target[Symbol.iterator](), keySelector));
    }
    ...
    GroupBy<K, E, R>(selKey: (T) => K, selElement: (T) => E = selfFn,
                     selResult: (a: K, b: Iterable<E>) => R = defGrouping): Enumerable<R> {
    ...
    SelectMany<S, V>(selector: (T, number) => Iterable<S> = selfFn, result: (T, S) => V = (t, s) => s): Enumerable<V> {
    ...
    Union<K>(second: Iterable<T>, keySelector: (T) => K = selfFn): Enumerable<T> {
        var aggregate = [this._target, second];
        return new Linq<T>(this, () => new UnionIteratror(aggregate[Symbol.iterator](), keySelector));
    }
...
class DistinctIteratror<T> extends MethodIteratror<T> implements Iterator<T> {
...
class UnionIteratror<T, K> extends SelectManyIteratror<T, T, T> implements Iterator<T> {
